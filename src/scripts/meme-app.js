...
async function generateMeme() {
  ...
  if (isWebShareSupported()) {
    try {
      const file = await fileFromUrl({ url: dataUrl, filename, mimeType: 'image/png' }).catch(err => {
        toastify(`<strong>Error preparing meme for sharing</strong><br><small>${err.message}</small>`, {
          trustDangerousInnerHTML: true, variant: 'danger'
        });
      });
  ...
  window.requestAnimationFrame(() => { downloadModal.open = true; });
}
...
function handleOpenVideoModalButtonClick() { videoModal.open = true; }

function handleTextPropChange(element, textboxId, prop) {
  const textboxData = Textbox.getById(textboxId).getData();
  switch (element.type) {
    case 'checkbox': textboxData[prop] = element.checked; break;
    case 'number': textboxData[prop] = Number(element.value); break;
    default: textboxData[prop] = element.value;
  }
  canvas.draw(selectedImage, Textbox.getAll());
}

function handleAddTextboxBtnClick() { return Textbox.create(); }
...
async function handleImageUploadFromURL(evt) {
  ...
  try {
    const file = await fileFromUrl({ url: imageUrl }).catch(() => {
...
function moveTextUsingArrowbuttons(textboxId, direction) {
  return function () {
    ...
    switch (direction) {
      case 'up': textboxData.offsetY -= 1; offsetYInput.value = textboxData.offsetY; break;
      case 'down': textboxData.offsetY += 1; offsetYInput.value = textboxData.offsetY; break;
      case 'left': textboxData.offsetX -= 1; offsetXInput.value = textboxData.offsetX; break;
      case 'right': textboxData.offsetX += 1; offsetXInput.value = textboxData.offsetX; break;
    }
    canvas.draw(selectedImage, Textbox.getAll());
...
function handleTextboxesContainerInput(evt) {
  const inputMap = {
    text: 'text', fillColor: 'fillColor', strokeColor: 'strokeColor', font: 'font',
    fontSize: 'fontSize', fontWeight: 'fontWeight', textAlign: 'textAlign', shadowBlur: 'shadowBlur',
    offsetY: 'offsetY', offsetX: 'offsetX', rotate: 'rotate', strokeWidth: 'strokeWidth',
    textBackgroundEnabled: 'textBackgroundEnabled', textBackgroundColor: 'textBackgroundColor'
  };
...
function handleTextboxesContainerChange(evt) {
  ...
  if (prop) handleTextPropChange(element, textboxId, prop);
}
...
function handleTextboxesContainerClick(evt) {
  ...
  if (element.matches('[data-button="delete-text-box"]')) {
    ...
    if (textboxToDelete && textboxToDelete.data.text.trim()) {
      ...
      if (textboxIdInput) { textboxIdInput.value = textboxId; removeConfirmationModal.open = true; }
    } else {
...
function handleTextRemoveFormSubmit(evt) {
  ...
  if (textboxId) { Textbox.remove(textboxId); removeConfirmationModal.open = false; }
}
...
function handleTextboxesContainerPointerup(evt) {
  if (evt.target.matches('[data-action="move-text"]')) {
...
async function handleGalleryClick(evt) {
  ...
  try {
    const file = await fileFromUrl({ url: img.src }).catch(() => {
      toastify(errorMessage, { trustDangerousInnerHTML: true, variant: 'danger' });
    });
    if (file) handleFileSelect(file);
  } catch {
...
function handleEmojiPickerSelection(evt) {
  ...
  if (textboxEl) {
    ...
    if (input) insertTextAtCursor(input, emoji);
  }
}
...
function handleMaxImageDimensionsFormChange(evt) {
  ...
  if (!selectedImage || isSolidColorSelected(selectedImage)) return;
  setImageMaxDimensions(selectedImage);
  canvas.draw(selectedImage, Textbox.getAll());
}
...
function handleTextboxCreate(evt) {
  ...
  if (textbox.getData().text) canvas.draw(selectedImage, Textbox.getAll());
}
...
function toggleTorchButtonStatus(options = {}) {
  const defaults = { el: document.getElementById('torchButton'), isTorchOn: false };
...
function handleTorchButtonClick(evt) {
  if (capturePhotoEl === null) return;
  capturePhotoEl.torch = !capturePhotoEl.torch;
  toggleTorchButtonStatus({ el: evt.currentTarget, isTorchOn: capturePhotoEl.hasAttribute('torch') });
}
...
function handleCameraSelectChange(evt) {
  if (!capturePhotoEl || typeof capturePhotoEl.restartVideoStream !== 'function' || capturePhotoEl.hasAttribute('loading')) return;
  capturePhotoEl.restartVideoStream(evt.target.value || undefined);
}
...
function changesHaveOccurred() {
  ...
  for (const v of Textbox.getAll().values()) {
    if (!Textbox.hasDefaultValues(v.data)) { hasChanges = true; break; }
  }
  return hasChanges;
}
