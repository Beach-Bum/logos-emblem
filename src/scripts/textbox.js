...
class Textbox {
  ...
  static createElement(textbox, autoFocus = true) {
    ...
    const {
      id, text, fillColor, strokeColor, fontSize, shadowBlur,
      strokeWidth, offsetX, offsetY, rotate, textBackgroundColor
    } = data;
    ...
    <div class="d-flex align-items-center" data-section="basic-settings">
      ...
      <div class="d-flex align-items-center pe-2">
        <label for="fillColorInput_${id}" class="visually-hidden">Fill color</label>
        <input class="form-control" type="color" value="${fillColor}" id="fillColorInput_${id}" data-input="fillColor" title="Fill color">

        <label for="strokeColorInput_${id}" class="visually-hidden">Outline color</label>
        ...
        <div class="col-4 mb-3">
          ...
          <select class="form-select" data-input="font" id="fontInput_${id}">
            ...
            <optgroup label="Custom fonts">
              ${customFonts.map(({ name, label }) => `<option value="${name}">${label}</option>`).join('')}
            </optgroup>
          </select>
        </div>
